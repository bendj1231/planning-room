...
const App: React.FC = () => {
  ...
  // Graphics Quality State - Default to MEDIUM as requested
  ...
  // View State (Zoom & Pan)
  // Start zoomed out (0.35) to show the entire board initially
                ...
                <div className="flex flex-col gap-2">
                     ...
                     <button 
                        onClick={() => setGraphicsQuality('HIGH')} 
                         className={`flex items-center justify-between p-3 rounded-lg border transition-all ${graphicsQuality === 'HIGH' ? 'bg-emerald-500/10 border-emerald-500/50 text-emerald-400' : 'bg-black/20 border-white/5 text-zinc-400 hover:bg-white/5'}`}
                     >
                        <div className="flex items-center gap-3">
                           <Sparkles size={18} />
                           <div className="text-left">
                               <div className="font-bold text-sm">High (Cinematic)</div>
                               <div className="text-[10px] opacity-70">High-end PCs, Gaming Rigs & Desktops</div>
                           </div>
                        </div>
                        {graphicsQuality === 'HIGH' && <Check size={16} />}
                     </button>

                     {/* Medium */}
                     <button 
                        onClick={() => setGraphicsQuality('MEDIUM')} 
                         className={`flex items-center justify-between p-3 rounded-lg border transition-all ${graphicsQuality === 'MEDIUM' ? 'bg-blue-500/10 border-blue-500/50 text-blue-400' : 'bg-black/20 border-white/5 text-zinc-400 hover:bg-white/5'}`}
                     >
                        <div className="flex items-center gap-3">
                           <Laptop size={18} />
                           <div className="text-left">
                               <div className="font-bold text-sm">Medium (Balanced)</div>
                               <div className="text-[10px] opacity-70">Mid-range PCs & Modern Laptops</div>
                           </div>
                        </div>
                        {graphicsQuality === 'MEDIUM' && <Check size={16} />}
                     </button>

                     {/* Low */}
                     <button 
                        onClick={() => setGraphicsQuality('LOW')} 
                        className={`flex items-center justify-between p-3 rounded-lg border transition-all ${graphicsQuality === 'LOW' ? 'bg-yellow-500/10 border-yellow-500/50 text-yellow-400' : 'bg-black/20 border-white/5 text-zinc-400 hover:bg-white/5'}`}
                     >
                        <div className="flex items-center gap-3">
                           <Zap size={18} />
                           <div className="text-left">
                               <div className="font-bold text-sm">Low (Performance)</div>
                               <div className="text-[10px] opacity-70">Low-end PCs & Older Laptops</div>
                           </div>
                        </div>
                        {graphicsQuality === 'LOW' && <Check size={16} />}
                     </button>
                </div>

                <button 
                    onClick={() => {
                        setShowIntroSettings(false);
                        setView(prev => ({ ...prev, scale: 0.6 })); // Zoom in for better editing experience on confirm
                    }}
                    className="mt-2 w-full py-3 bg-white text-black font-bold rounded-lg hover:bg-zinc-200 transition-colors shadow-lg active:scale-95 flex items-center justify-center gap-2"
                >
                    Confirm Settings
                </button>
            </div>
