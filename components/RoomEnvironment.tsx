  ...
  return (
    <div className="relative w-full h-screen bg-[#050505] flex items-center justify-center overflow-hidden">
      {/* 
         ...
         - Low Quality: Uses a single radial gradient to simulate a vignette/lit room efficiently.
         - Med/High: Uses multiple layers for spotlight, wall texture, and ambient occlusion.
      */}
      
      {isLow ? (
        // Low Quality: Simple Vignette Gradient (Light center -> Dark edges)
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,#1a1a1a_0%,#000000_100%)] pointer-events-none z-0"></div>
      ) : (
        <>
          {/* The "Spotlight" on the wall */}
          ...
          {/* Realistic Wall Texture (Concrete/Plaster) */}
